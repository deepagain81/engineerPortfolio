import { Card, Container } from '@/components'
import { SECTION_IDS, site } from '@/data'

export function Skills() {
  return (
    <section id={SECTION_IDS.skills} className="py-12">
      <Container>
        <h2 className="t-section">{site.sections.skills.title}</h2>
        <p className="t-muted mt-2">{site.sections.skills.description}</p>

        <div className="mt-6 grid gap-4 md:grid-cols-2">
          {site.skills.map((skill) => (
            <Card key={skill.group}>
              <h3 className="text-sm font-semibold">{skill.group}</h3>
              <ul className="mt-3 grid gap-2 text-sm text-fg/90 sm:grid-cols-2">
                {skill.items.map((item) => (
                  <li key={item} className="rounded-lg bg-fg/5 px-3 py-2">
                    {item}
                  </li>
                ))}
              </ul>
            </Card>
          ))}
        </div>
      </Container>
    </section>
  )
}
